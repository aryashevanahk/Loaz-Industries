...
// Get active services with technician (only services with assigned technician)
$stmt = $pdo->prepare("
    SELECT s.id, s.device, s.status, t.user_id as technician_user_id, u.name as technician_name
    FROM services s
    JOIN technicians t ON s.technician_id = t.id
    JOIN users u ON t.user_id = u.id
    WHERE s.user_id = ? AND s.technician_id IS NOT NULL AND s.status != 'done'
...
$selected_technician = null;

if ($selected_service > 0) {
    $stmt = $pdo->prepare("
        SELECT s.*, t.user_id as technician_user_id, u.name as technician_name
        FROM services s
        JOIN technicians t ON s.technician_id = t.id
        JOIN users u ON t.user_id = u.id
        WHERE s.id = ? AND s.user_id = ?
    ");
    $stmt->execute([$selected_service, $_SESSION['user_id']]);
    $selected_technician = $stmt->fetch();
}
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                <div>
                    ...
                    <h1 class="display-5 fw-light" style="color: var(--dark-brown);">
                        <i class="fas fa-comments me-2" style="color: var(--gold-brown);"></i>Konsultasi Teknisi
                    </h1>
                    <p class="text-muted">Diskusikan masalah elektronik Anda dengan teknisi</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-4 mb-4 mb-md-0">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-transparent border-0 pt-4">
                    <h5 class="mb-0"><i class="fas fa-users me-2" style="color: var(--gold-brown);"></i>Konsultasi Aktif</h5>
                </div>
                <div class="card-body p-0">
                    <?php if (empty($active_services)): ?>
                        <div class="text-center py-5">
                            <i class="fas fa-comment-slash fa-3x text-muted mb-3"></i>
                            <p class="text-muted">Belum ada konsultasi aktif</p>
                            <a href="request_service.php" class="btn btn-gold rounded-4 mt-2">
                                   ...
                                   class="list-group-item list-group-item-action border-0 px-4 py-3 <?php echo ($selected_service == $service['id']) ? 'active-chat' : ''; ?>">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1">
                                            <h6 class="mb-0"><?php echo htmlspecialchars($service['technician_name']); ?></h6>
                ...
                <div class="card border-0 shadow-sm rounded-4">
                    <div class="card-header bg-transparent border-0 pt-4 pb-0">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <h5 class="mb-0"><?php echo htmlspecialchars($selected_technician['technician_name']); ?></h5>
                                <small class="text-muted">
                                    <i class="fas fa-microchip me-1"></i> <?php echo htmlspecialchars($selected_technician['device']); ?>
                                    <span class="mx-1">•</span>
                                    <span class="badge bg-<?php 
                                        echo $selected_technician['status'] == 'pending' ? 'warning' : 
                                            ($selected_technician['status'] == 'accepted' ? 'info' : 
                                            ($selected_technician['status'] == 'repairing' ? 'primary' : 'success')); 
                                    ?> rounded-pill">
                                        <?php echo ucfirst($selected_technician['status']); ?>
                                    </span>
                                </small>
                            </div>
                        </div>
                    ...
                    <div class="card-body p-0">
                        ...
                        <div class="chat-input border-top p-4">
                            <form id="chatForm" class="d-flex gap-2">
                                <input type="hidden" id="service_id" value="<?php echo $selected_technician['id']; ?>">
                                <input type="hidden" id="technician_id" value="<?php echo $selected_technician['technician_user_id']; ?>">
                                <input type="text" id="messageInput" class="form-control rounded-4" placeholder="Tulis pesan Anda..." autocomplete="off">
            ...
            <?php else: ?>
                <div class="card border-0 shadow-sm rounded-4 text-center py-5">
                    <div class="card-body">
                        ...
                        <h4>Pilih Konsultasi</h4>
                        <p class="text-muted">Pilih layanan servis aktif dari menu di samping untuk mulai berkonsultasi dengan teknisi</p>
...
<script>
    ...
    function renderMessages(messages) {
        ...
        if (!messages || messages.length === 0) {
            html = '<div class="text-center text-muted py-5"><i class="fas fa-comment-dots fa-3x mb-2"></i><br>Belum ada pesan. Mulai konsultasi dengan teknisi!</div>';
        } else {
    ...
    function sendTypingStatus() {
        ...
        let serviceId = $('#service_id').val();
        $.post('../api/typing_status.php', { service_id: serviceId, typing: true });
        
        clearTimeout(typingTimeout);
        typingTimeout = setTimeout(function() {
            isTyping = false;
            $.post('../api/typing_status.php', { service_id: serviceId, typing: false });
        }, 1000);
    }
